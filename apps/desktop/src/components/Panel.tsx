  ...
  return (
    <section style={{ ...shellStyle, padding: dense ? "16px" : shellStyle.padding }}>
      {(title || eyebrow || description || actions) && (
        <header style={{ display: "flex", alignItems: "start", justifyContent: "space-between", gap: "16px", marginBottom: "16px" }}>
          <div>
            {eyebrow ? <div style={{ color: theme.colors.accent, fontSize: "12px", letterSpacing: "0.14em", textTransform: "uppercase" }}>{eyebrow}</div> : null}
            {title ? <h2 style={{ margin: "6px 0 4px", fontSize: "20px", lineHeight: 1.2 }}>{title}</h2> : null}
