...
- 实现批次、集中 review、批量修复、focused checks 和回归节奏见 `development-execution.md`。
...
方案设计阶段的 reviewer 默认只读。涉及写入时，遵守 `development-execution.md` 的 coordinator/subagent 规则，避免多个 agent 写同一组文件。
...
每个 execution batch 必须追溯到 goal，且不能把 batch 完成误报为 goal complete。具体实现节奏遵守 `development-execution.md`。
