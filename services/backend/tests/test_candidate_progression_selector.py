...
from scene_pilot.services.candidate_progression_selector import (
    ...
    is_application_progression_eligible,
    map_application_progression_task_type,
...
def _target(
    ...
    *,
    ...
    return ApplicationProgressionTarget(
...
def test_selector_prefers_highest_composite_candidate() -> None:
    ...
    selected = select_next_application_progression(
        [
            _target(
                "candidate-high-wait",
                "outreach_pending",
    ...
    assert selected.application_id == "candidate-high-wait"
...
def test_interview_pending_ai_auto_can_enter_autonomous_pool() -> None:
    ...
    assert is_application_progression_eligible(target, now=now) is True
    assert map_application_progression_task_type(target) == "candidate_outreach"
...
def test_offline_scoring_human_required_cannot_enter_autonomous_pool() -> None:
    ...
    assert is_application_progression_eligible(target, now=now) is False
    assert map_application_progression_task_type(target) is None
...
def test_selector_skips_open_task_and_prefers_next_candidate() -> None:
    ...
    assert selected is not None
    assert selected.application_id == "candidate-fallback"
