...
def _build_jd_sync_user_instruction(
    *,
    ...
    instruction_parts = [
        ...
        *(
            [
                "- 本次运行模式是单 JD 试跑：最多同步 1 个招聘中 JD；只要已完整读取详情文本、写入本地 JD 并验证成功，本阶段即可收敛，不要求完成全量职位发现、全量详情读取或更新/下架识别。",
            ]
            if single_probe
            else []
        ),
        "- JD sync 完成条件是：真实职位详情文本已观察，本地 JD 写回已由工具返回或本地二次读取确认，且 pending_jobs 为空。",
        ...
        *(
            ...
            if single_probe
                ...
                "- 可以先写入已经完整读取详情的职位作为进度；但在没有完成全量职位发现、全量详情读取、更新/下架识别且 pending_jobs 为空前，不得用“已完成部分同步”结束本轮。",
            ]
        ),
        "- 只发现和同步 JD。",
        "- 不处理候选人筛选、评分、外联或投递推进。",
    ...
    instruction_parts.extend(
        [
            "",
            "同步完成后：",
            "- 输出本轮已观察真实详情文本、已写回本地 JD、pending_jobs 为空的验证摘要。",
        ]
    )
    if user_instruction:
...
def _boss_zhipin_ui_sop_lines() -> list[str]:
    return [
        "- 顶层页面恢复必须使用 BOSS 主导航可见入口；JD sync 只允许通过 BOSS 主导航“职位管理”恢复入口。推荐牛人、搜索、沟通仅作为页面识别锚点或后续非 JD sync 招聘自动化入口；招聘规范、我的客服、面试、招聘数据、账号权益、升级VIP 等顶部栏/权益入口不是本工作流入口。",
