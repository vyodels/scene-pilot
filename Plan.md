...
截至 `2026-04-15`，当前仓库已经完成这一波本地可交付的重构目标：
...
- 用户态术语已统一：
  ...
  - `task` -> `工作流`
  - `task instance` -> `工作流实例`
- 正式的 recruiting 工作流验证已完成：
  - 自然语言工作流创建成功
  - `llm_structured` 编译成功
  - supervised trial 成功进入 `awaiting_review`
  - 确认后可激活工作流版本
  - 正式运行在缺少 live browser scene snapshot 时，会直接进入 `waiting_human / awaiting_review`
  - 不再错误进入 executor 并打成 `timeout / Token budget exceeded`
- 本地 OpenAI-compatible provider 路径已验证可用于正式联调：
  - `http://127.0.0.1:8317/v1`
  - 模型：`gpt-5.4`
  - 当前后端可从持久化 settings 自动恢复该 provider 配置
